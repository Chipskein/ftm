...
try
    dados_brutos = csvread('comparacao-yolo-1024-v8sseg-matrix-confusao-manual.csv', 1, 1);
catch
    error('Certifique-se de que o arquivo comparacao-yolo-1024-v8sseg-matrix-confusao-manual.csv esta na mesma pasta!');
end
...
legend('YOLO V8Seg', 'YOLO 1024px', 'location', 'northeastoutside');
grid on;
ylim([0 1.2]); % Espaço para a legenda
saveas(gcf, 'comparacao-yolo-1024-v8sseg-metricas.png');

% 8. Exibir resultados no Terminal para copiar para o texto do TCC
fprintf('\n================ RESULTADOS FINAIS ================\n');
fprintf('Imagens processadas: %d\n', size(dados, 1));
fprintf('---------------------------------------------------\n');
fprintf('MODELO V8Seg  | P: %.4f | R: %.4f | F1: %.4f\n', p3, r3, f1_3);
